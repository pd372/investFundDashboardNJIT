...
def getStockBeta(ticker, years=False):
...
def getStockBetaBr(ticker, years=False):
    #getting 'years' years of data
    if not years:
        today=date.today()
        td=str(today)
...
def getPortBetaBr(df, weights):
    betas=[]
    for sec in df.columns:
        b=getStockBetaBr(sec, 5)
        betas.append(b)
...
#Adjust the CAPM for a company listed in Brazil
def getCAPMBr(ticker):
    Ri=getCAPM(ticker)
    
    
    cds=0.0308#consider the cds for brazil as 3.54%
    Ri=Ri+cds #adding the country risk premium 
    
    #BR data
    data=quandl.get("BCB/433", authtoken="rUjnjGxLLbzuUKzaBmdQ")#IPCA
    data=data[::-1][:12]# just the last year of data
    infBr=data.mean()*12/100 #annualized
    
    #US data
    today=date.today()
    td=str(today)
    curYr=int(td[:4])
    data2=web.DataReader('T5YIE','fred',start='{}-1-1'.format(curYr))
    infUs=float(data2.iloc[-1])/100

    Ke=(1+Ri)*((1+infBr)/(1+infUs))-1
    return Ke
...
def maximizeSharpeRatio(df, reps=False):  
...
def minimizeRisk(df, reps=False):
    rsks={}
    minRsk=np.inf
    if not reps:
...
def runDCF(ticker):
    url = ("https://financialmodelingprep.com/api/v3/company/discounted-cash-flow/{}?apikey=80abdf356424b53192f75a0aa1a6e2e8".format(ticker))    
    response = urlopen(url)
    data = response.read().decode("utf-8")
    dcf=json.loads(data)
    df=pd.DataFrame(dcf.items(),columns=['Index','Values']).set_index(['Index'])
    df.loc['Gain(loss)%']=(df.loc['dcf']-df.loc['Stock Price'])/df.loc['Stock Price']*100
    return df
...
#Use to rebalance the portfolio without having to create a new one
def portRebalance (df,buy=False,sell=False,day=False):  
    ...
    if buy:
        ...
        newP=createPortfolio(df.columns.tolist()+buy, start=day)
        
        rebalPort= df.append(newP, sort=False)
        
        rebalPort['date']=rebalPort.index
        
        rebalPort.drop_duplicates(subset='date',keep='last', inplace=True)
        
        rebalPort.drop('date', 1, inplace=True)
        
        rebalPort.fillna(0, inplace=True)
        
        cashProc -= np.sum(rebalPort[buy].iloc[-1])
        
        return  rebalPort, cashProc

    if sell:
        
        cashProc += np.sum(df[sell].iloc[-1])
